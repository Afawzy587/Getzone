...
$(document).on("click", "a.deleteThisMembership", function (e) {
	e.preventDefault();
	x = confirm("هل أنت متأكد أنك تود مسح هذه العضوية من جهاز الباركود ؟");
	if(x == true)
	{
		id = $(this).attr('membership');
		jQuery.ajax( {
			async :true,
			type :"POST",
			url :"cards.html?do=delete_m",
			data: "id=" + id + "",
			success : function(data) {
				if(data == 116)
				{
					$(".membership_" + id).fadeTo(400, 0, function () { $(".membership_" + id).slideUp(400);});
				}
			},
			error : function() {
				return true;
...
$(document).on("click", "a.useThisMembership", function (e) {
	e.preventDefault();
	id = $(this).attr('membership');
	jQuery.ajax( {
		async :true,
		type :"POST",
		url :"cards.html?do=use_m",
		data: "id=" + id + "",
		success : function(data) {
			if(data == 116)
			{
				alert("تم إختيار هذه العضوية للإستخدام");
			}
		},
		error : function() {
			return true;
